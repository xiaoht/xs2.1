...
$(function() {
    ...
    $('.buy-box div').click(function() {
        ...
        $('#dgscBtn').unbind("click").on('click', function() {
            ...
            if (that == 1 || pandunFq) {
		var urlHeader_test = ajaxLink + "v1/site/test";
                var dataParam_test;
                dataParam_test = "user_id="+user_id+"&token="+token;
                function sucTest(response) {
                    getTips('登录状态异常');
                }
                ajaxWithHeader(urlHeader_test, dataParam_test, sucTest,'get');
                if (user_id && token) {
