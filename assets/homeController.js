makerApp.controller('homeController', ['$scope', '$http', '$state', '$ionicSlideBoxDelegate', '$ionicViewSwitcher',
    ...
    // localStorage.clear();
        ...
        $scope.$on('$ionicView.enter',function () {
            ...
            $scope.getColoum();
            console.log('屏幕之宽度为'+window.screen.width);
            ...
            console.log('屏幕宽度之五分之一为：'+$scope.oneFithOfScreenWidth);
        })
        $scope.remain = [];  //倒计时
        ...
        function getRecommendList () {
            ...
            bToCService.getHomeRecommendList(param).then(function (data) {
                data = data.data;
                if(data.ret == 0){
                    $scope.homeReocmmendList = $scope.homeReocmmendList.concat(data.data);
